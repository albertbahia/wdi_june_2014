...
function Cat(name, breed, furColor, weight, age) {
  ...
  this.readCollar = function() {
    var collar = "Name: " + this.name + " // " + "Age: " + this.age + " // " + this.weight + "lbs" + " // " + this.furColor + " " + this.breed;
    return collar;
  };

  this.lick = function() {
    if {(hairballsBeforeLicking = cat.hairballs.length)
      returns cat.lick;
  }  else {(hairballsAfterLickingThrice = cat.hairballs.length)
    returns cat.hairballs.length = 0;
  }
}
