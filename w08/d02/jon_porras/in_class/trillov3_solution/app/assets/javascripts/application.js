...
function renderCompleted(card) {
  ...
  var description = $("<span class='description'>"+card.description+"</span>")
  listItem.append(deleteSpan).append(description);
...
function renderTodo(card) {
  ...
  var description = $("<span class='description'>"+card.description+"</span>")
  listItem.append(description).append(deleteSpan).append(finishSpan)
...
function keypressHandler(event, fxn) {
  if(event.which===13) {
    fxn();
  }
}
