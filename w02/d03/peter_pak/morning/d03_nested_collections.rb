...
# def list_info
#   pokedex.map { |key| key[:name] }
# end
#
# def names_id(pokedex)
#   names_id = Hash[pokedex.map { |hash| hash.values_at(:name, :national_id) } ]
# end
...
def poke_parser(poke_info)
  all_pokemon = poke_info.map do |poke_hash|
    ...
    types = poke_hash[:types][0].values.map {|word| word.capitalize}.join(' // ')

    info_string = "Name: #{name} | ID: #{id}\n"
