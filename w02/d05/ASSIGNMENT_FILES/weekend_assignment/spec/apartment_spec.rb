...
describe Apartment do
  ...
  describe '#list_tenants' do
    it 'lists all of the tenants information' do
      ...
      apartment.add_tenants(tenant_2)
      expect(apartment.list_tenants).to eq("Name: Joe, Age: 25, Gender: Male\nName: Jorb, Age: 9000, Gender: Female")
    end
  ...
  describe '#add_tenant' do
    it 'adds a tenant to the apartment' do
      apartment.add_tenants(tenant)
      apartment.add_tenants(tenant_2)
      apartment.add_tenants(tenant_3)
      expect(apartment.tenants).to eq([tenant, tenant_2])
    end
    it 'does not add more tenants than rooms' do
      tester = Tenant.new("Testguy", 32, "Male")
      apartment.add_tenants(tenant)
      apartment.add_tenants(tenant_2)
      apartment.add_tenants(tenant_3)
      apartment.add_tenants(tester)
      expect(apartment.tenants).to eq([tenant, tenant_2])
    end
  end
